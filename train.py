...
def main_worker(args):
    ...
    # REC任务需要find_unused_parameters=True，因为proj模块不参与计算
    task = getattr(args, 'task', 'ris')
    find_unused = (task == 'rec')
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if dist.get_rank() == 0:
            # 根据任务类型选择保存的指标名称
            task = getattr(args, 'task', 'ris')
            metric_name = 'Acc@50' if task == 'rec' else 'IoU'
    ...
    task = getattr(args, 'task', 'ris')
    if task == 'rec':
        logger.info("* Best Acc@50={:.2f}% * ".format(best_IoU * 100))
    else:
