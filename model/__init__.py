...
def build_segmenter(args):
    ...
    # REC任务时打印box_head参数
    if task == 'rec' and box_head:
        n_box_head_parameters = sum(p.numel() for p in box_head)
        logger.info(f'number of updated params (box_head): {n_box_head_parameters}')
